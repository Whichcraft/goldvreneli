...
This file consolidates the previous `TODO.md`, `todo.md`, and
...
## Historical Backlog (Imported From `todo.md`)

This section preserves the previous lowercase backlog for reference.
The prioritized backlog above is the active list. Imported items below may
contain stale status labels because the old file mixed open and completed work
...
## Imported Notes (From `TODO-IMPROVEMENTS.md`)

This section preserves the previous short improvement note file.

1. [DONE] Refactor broker API calls to use AsyncIO to prevent UI blocking.
2. [DONE] Optimize Streamlit state management to reduce unnecessary re-renders.
3. [LOW] Implement a shared in-memory caching layer for multi-broker data.
